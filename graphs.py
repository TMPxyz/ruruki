...
class Graph(interfaces.IGraph):
    ...
    def add_vertex(self, label=None, vclass=None, **kwargs):
        if vclass is None:
            vclass = self._vclass
        vertex = vclass(label=label, **kwargs)
    ...
    def _vertex_constraint_violated(self, vertex, **kwargs):
        ...
        :raises ConstraintViolation: Raised if you a constraint violation has
            been found.
        """
        # my fix: this will create an empty {} whatever, a waste of time 
        # key_index = self._vconstraints.get(vertex.label, {})
        key_index = self._vconstraints.get(vertex.label) or {}
    ...
    def remove_all_edges_of_vertex(self, vtx): # my add
        "remove all edges of given vertex"
        es = vtx.get_both_edges().all()
        for e in es:
            self.remove_edge(e)
