...
class Entity(interfaces.IEntity):
    ...
    """
    __slots__ = ["ident", "label", "properties", "graph", "extras"]
    ...
    def __init__(self, label=None, **kwargs):
        ...
        self.extras = {} # add extra save space that has nothing to do with indexing
...
class EntitySet(interfaces.IEntitySet):
    ...
    def update_index(self, entity, **kwargs):
        # my fix: setdefault will call the D part even if there is hit
        ...
        collection = self._prop_reference.get(entity.label)
        if None == collection:
            collection = {"_all": set() }
            self._prop_reference[entity.label] = collection
